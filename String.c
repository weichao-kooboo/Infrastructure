#include "Config.h"
static u_char *hky_sprintf_num(u_char *buf, u_char *last, uint64_t ui64,
	u_char zero, hky_uint_t hexadecimal, hky_uint_t width);
static void hky_encode_base64_internal(hky_str_t *dst, hky_str_t *src,
	const u_char *basis, hky_uint_t padding);
static hky_int_t hky_decode_base64_internal(hky_str_t *dst, hky_str_t *src,
	const u_char *basis);
...
u_char * hky_cdecl
hky_sprintf(u_char *buf, const char *fmt, ...)
{
	u_char   *p;
	va_list   args;
...
u_char * hky_cdecl
hky_snprintf(u_char *buf, size_t max, const char *fmt, ...)
{
	u_char   *p;
	va_list   args;
...
u_char * hky_cdecl
hky_slprintf(u_char *buf, u_char *last, const char *fmt, ...)
{
	u_char   *p;
	va_list   args;
...
u_char *
hky_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
	u_char                *p, zero;
	int                    d;
	double                 f;
...
hky_int_t
hky_strcasecmp(u_char *s1, u_char *s2)
{
	hky_uint_t  c1, c2;
...
hky_int_t
hky_strncasecmp(u_char *s1, u_char *s2, size_t n)
{
	hky_uint_t  c1, c2;
...
u_char *
hky_strnstr(u_char *s1, char *s2, size_t len)
{
	u_char  c1, c2;
	size_t  n;
	...
	} while (hky_strncmp(s1, (u_char *)s2, n) != 0);

	return --s1;
}

u_char *
hky_strstrn(u_char *s1, char *s2, size_t n)
{
	u_char  c1, c2;
...
u_char *
hky_strcasestrn(u_char *s1, char *s2, size_t n)
{
	hky_uint_t  c1, c2;
	...
	} while (hky_strncasecmp(s1, (u_char *)s2, n) != 0);
...
u_char *
hky_strlcasestrn(u_char *s1, u_char *last, u_char *s2, size_t n)
{
	hky_uint_t  c1, c2;
...
hky_int_t
hky_rstrncmp(u_char *s1, u_char *s2, size_t n)
{
	if (n == 0) {
		return 0;
	}
...
hky_int_t
hky_rstrncasecmp(u_char *s1, u_char *s2, size_t n)
{
	u_char  c1, c2;
...
hky_int_t
hky_memn2cmp(u_char *s1, u_char *s2, size_t n1, size_t n2)
{
	size_t     n;
	hky_int_t  m, z;
...
hky_int_t
hky_dns_strcmp(u_char *s1, u_char *s2)
{
	hky_uint_t  c1, c2;
...
hky_int_t
hky_filename_cmp(u_char *s1, u_char *s2, size_t n)
{
	hky_uint_t  c1, c2;
...
/* parse a fixed point number, e.g., hky_atofp("10.5", 4, 2) returns 1050 */

hky_int_t
hky_atofp(u_char *line, size_t n, size_t point)
{
	hky_int_t   value, cutoff, cutlim;
	hky_uint_t  dot;
...
ssize_t
hky_atosz(u_char *line, size_t n)
{
...
off_t
hky_atoof(u_char *line, size_t n)
{
...
time_t
hky_atotm(u_char *line, size_t n)
{
...
hky_int_t
hky_hextoi(u_char *line, size_t n)
{
	u_char     c, ch;
...
u_char *
hky_hex_dump(u_char *dst, u_char *src, size_t len)
{
...
uint32_t
hky_utf8_decode(u_char **p, size_t n)
{
	size_t    len;
	uint32_t  u, i, valid;
...
size_t
hky_utf8_length(u_char *p, size_t n)
{
	u_char  c, *last;
	size_t  len;
...
u_char *
hky_utf8_cpystrn(u_char *dst, u_char *src, size_t n, size_t len)
{
	u_char  c, *next;
...
uintptr_t
hky_escape_uri(u_char *dst, u_char *src, size_t size, hky_uint_t type)
{
	hky_uint_t      n;
	uint32_t       *escape;
	static u_char   hex[] = "0123456789ABCDEF";
...
void
hky_unescape_uri(u_char **dst, u_char **src, size_t size, hky_uint_t type)
{
...
uintptr_t
hky_escape_html(u_char *dst, u_char *src, size_t size)
{
	u_char      ch;
	hky_uint_t  len;
...
uintptr_t
hky_escape_json(u_char *dst, u_char *src, size_t size)
{
	u_char      ch;
	hky_uint_t  len;
...
void
hky_str_rbtree_insert_value(hky_rbtree_node_t *temp,
...
/* hky_sort() is implemented as insertion sort because we need stable sort */

void
hky_sort(void *base, size_t n, size_t size,
	hky_int_t(*cmp)(const void *, const void *))
{
	u_char  *p1, *p2, *p;

	p = hky_alloc(size, hky_origin->log);
	if (p == NULL) {
		return;
	}
...
static u_char *
hky_sprintf_num(u_char *buf, u_char *last, uint64_t ui64, u_char zero,
	hky_uint_t hexadecimal, hky_uint_t width)
{
	u_char         *p, temp[HKY_INT64_LEN + 1];
	/*
	* we need temp[HKY_INT64_LEN] only,
...
static void
hky_encode_base64_internal(hky_str_t *dst, hky_str_t *src, const u_char *basis,
	hky_uint_t padding)
{
	u_char         *d, *s;
	size_t          len;
...
static hky_int_t
hky_decode_base64_internal(hky_str_t *dst, hky_str_t *src, const u_char *basis)
{
	size_t          len;
	u_char         *d, *s;
